...
class _VideoEditorScreenState extends State<VideoEditorScreen>
  ...
  Future<void> _addFirstMusicFromLibrary() async {
    ...
    try {
      ...
      if (list is! List || list.isEmpty) {
        _showMsg('音乐库为空', isError: true);
        return;
      }
      final first = list.first;
      if (first is! Map) return;
      final url = first['fileUrl']?.toString();
      if (url == null || url.isEmpty) {
        _showMsg('首条音乐无 fileUrl', isError: true);
        return;
      }
  ...
  Future<void> _removeMusic() async {
    ...
    try {
      ...
      setState(() => _appliedMusicPath = null);
      _showMsg('已取消背景音乐');
    } catch (_) {
  ...
  Widget _buildAudioTab() {
    return ListView(
      padding: const EdgeInsets.all(16),
      children: [
        _buildSectionTitle('背景音乐与音量', Icons.music_note_rounded),
        const SizedBox(height: 10),
        _buildSliderRow('音量 ${(_volume * 100).toInt()}%', _volume, 0, 1.5,
  ...
  Widget _buildTransitionPicker() {
    final transitions = [
      {'id': 'fade', 'label': '淡入淡出', 'icon': Icons.blur_on_rounded, 'desc': '画面渐隐渐现，柔和过渡'},
      {'id': 'slide', 'label': '滑动', 'icon': Icons.swap_horiz_rounded, 'desc': '画面左右滑入切换'},
      {'id': 'zoom', 'label': '缩放', 'icon': Icons.zoom_in_rounded, 'desc': '画面由小放大过渡'},
      {'id': 'rotate', 'label': '旋转', 'icon': Icons.rotate_right_rounded, 'desc': '画面旋转切换场景'},
      {'id': 'wipe', 'label': '擦除', 'icon': Icons.cleaning_services_rounded, 'desc': '画面逐渐擦除过渡'},
    ];
        ...
        Container(
          ...
          child: Row(
            children: [
              ...
              Expanded(
                child: Text(
                  '${selectedInfo['label']}：${selectedInfo['desc']}',
                  style: TextStyle(
                    fontSize: 12,
